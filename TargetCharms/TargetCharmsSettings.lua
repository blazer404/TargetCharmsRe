...
function TargetCharms_InitSettings()
	...
	do
		...
		init:SetParentInitializer(tcEnabled, function() return tcEnabledSetting:GetValue() end)
	end
	...
	do
		local s = Settings.RegisterProxySetting(category, "TC_SCALE", Settings.VarType.Number, TARGETCHARMS_OPTIONS_SCALE, 1.0,
			...
			function(v) TargetCharms_Options["TargetCharms"]["barscale"] = v; SetFrameScale(v, 1) end)
		local o = Settings.CreateSliderOptions(0.02, 2.0, 0.02)
	...
	do
		local s = Settings.RegisterProxySetting(category, "TC_OPACITY", Settings.VarType.Number, TARGETCHARMS_OPTIONS_OPACITY, 1.0,
			...
			function(v) TargetCharms_Options["TargetCharms"]["alphaVal"] = v; TopCharm:SetAlpha(v) end)
		local o = Settings.CreateSliderOptions(0, 1, 0.05)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(tcEnabled, function() return tcEnabledSetting:GetValue() end)
	end
	...
	do
		local s = Settings.RegisterProxySetting(category, "TC_XSPACING", Settings.VarType.Number, TARGETCHARMS_OPTIONS_XSPACING, 0,
			...
			function(v) TargetCharms_Options["TargetCharms"]["Xspacing"] = v; SetupButtons("TargetCharms", "TargetCharms") end)
	...
	do
		local s = Settings.RegisterProxySetting(category, "TC_YSPACING", Settings.VarType.Number, TARGETCHARMS_OPTIONS_YSPACING, 0,
			...
			function(v) TargetCharms_Options["TargetCharms"]["Yspacing"] = v; SetupButtons("TargetCharms", "TargetCharms") end)
		local o = Settings.CreateSliderOptions(-20, 20, 1)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(tcEnabled, function() return tcEnabledSetting:GetValue() end)
	...
	do
		...
		local init = Settings.CreateControlInitializer("TargetCharmsEditBoxControlTemplate", s, nil, TARGETCHARMS_OPTIONS_LAYOUT_TEXT)
		init:SetParentInitializer(tcEnabled, function() return tcEnabledSetting:GetValue() end)
	...
	do
		...
		local function GetPresetOptions()
			...
			for _, v in ipairs(TargetCharms_LayoutDefaults) do
				container:Add(v[2], v[1], TARGETCHARMS_OPTIONS_LAYOUT_TEXT)
			end
			return container:GetData()
		end
		local init = Settings.CreateDropdown(category, s, GetPresetOptions, TARGETCHARMS_OPTIONS_LAYOUT_TEXT)
		init:SetParentInitializer(tcEnabled, function() return tcEnabledSetting:GetValue() end)
	end
	...
	do
		...
		local init = Settings.CreateCheckbox(category, s)
		init:SetParentInitializer(flEnabled, function() return flEnabledSetting:GetValue() end)
	end
	...
	do
		local s = Settings.RegisterProxySetting(category, "FL_SCALE", Settings.VarType.Number, TARGETCHARMS_OPTIONS_SCALE, 1.0,
			...
			function(v) TargetCharms_Options["FlareCharms"]["barscale"] = v; SetFrameScale(v, 5) end)
		local o = Settings.CreateSliderOptions(0.2, 2.0, 0.02)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(flEnabled, function() return flEnabledSetting:GetValue() end)
	end
	...
	do
		local s = Settings.RegisterProxySetting(category, "FL_OPACITY", Settings.VarType.Number, TARGETCHARMS_OPTIONS_OPACITY, 1.0,
			...
			function(v) TargetCharms_Options["FlareCharms"]["alphaVal"] = v; TopFlare:SetAlpha(v) end)
		local o = Settings.CreateSliderOptions(0, 1, 0.05)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(flEnabled, function() return flEnabledSetting:GetValue() end)
	...
	do
		local s = Settings.RegisterProxySetting(category, "FL_XSPACING", Settings.VarType.Number, TARGETCHARMS_OPTIONS_XSPACING, 0,
			...
			function(v) TargetCharms_Options["FlareCharms"]["Xspacing"] = v; SetupButtons("FlareCharms", "FlareCharms") end)
		local o = Settings.CreateSliderOptions(-20, 20, 1)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(flEnabled, function() return flEnabledSetting:GetValue() end)
	...
	do
		local s = Settings.RegisterProxySetting(category, "FL_YSPACING", Settings.VarType.Number, TARGETCHARMS_OPTIONS_YSPACING, 0,
			...
			function(v) TargetCharms_Options["FlareCharms"]["Yspacing"] = v; SetupButtons("FlareCharms", "FlareCharms") end)
		local o = Settings.CreateSliderOptions(-20, 20, 1)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(flEnabled, function() return flEnabledSetting:GetValue() end)
	...
	do
		...
		local init = Settings.CreateCheckbox(category, s)
		init:SetParentInitializer(rcEnabled, function() return rcEnabledSetting:GetValue() end)
	end
	...
	do
		local s = Settings.RegisterProxySetting(category, "RC_SCALE", Settings.VarType.Number, TARGETCHARMS_OPTIONS_SCALE, 1.0,
			...
			function(v) TargetCharms_Options["ReadyCharm"]["barscale"] = v; TopReady:SetScale(v) end)
		local o = Settings.CreateSliderOptions(0.02, 2.0, 0.02)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(rcEnabled, function() return rcEnabledSetting:GetValue() end)
	...
	do
		local s = Settings.RegisterProxySetting(category, "RC_OPACITY", Settings.VarType.Number, TARGETCHARMS_OPTIONS_OPACITY, 1.0,
			...
			function(v) TargetCharms_Options["ReadyCharm"]["alphaVal"] = v; TopReady:SetAlpha(v) end)
		local o = Settings.CreateSliderOptions(0, 1, 0.05)
		o:SetLabelFormatter(MinimalSliderWithSteppersMixin.Label.Right)
		local init = Settings.CreateSlider(category, s, o)
		init:SetParentInitializer(rcEnabled, function() return rcEnabledSetting:GetValue() end)
	...
	do
		...
		local init = Settings.CreateControlInitializer("TargetCharmsEditBoxControlTemplate", s, nil, TARGETCHARMS_OPTIONS_READYCHECK_TEXT)
		init:SetParentInitializer(rcEnabled, function() return rcEnabledSetting:GetValue() end)
		layout:AddInitializer(init)
	end
	...
	layout:AddInitializer(CreateSettingsListSectionHeaderInitializer(TARGETCHARMS_OPTIONS_RESET_TITLE))

	layout:AddInitializer(CreateSettingsButtonInitializer(
		TARGETCHARMS_PANEL_RESET, TARGETCHARMS_PANEL_RESET,
		function() TargetCharms_Reset() end, nil, true))
